...
if ( (!empty($_POST['uploadSummary']) || !empty($_POST['uploadArticle']) ) &&  ($_FILES['file']['error'] == 0) )
{
	...
	if (empty($error))
	{
		...
		if (!empty($_POST['uploadSummary'])){
			$working_up = resize_img($uploaded, $workingWdthSummary);
		}else{
...

{
	...
	else $error['sobreescribir'] = 'ya existe una imagen con este nombre';
	...
	<div style="padding: 20px; background-color: #e0e0e0; overflow: hidden;">
			...
			foreach ($images as $image){?>
				<div style="clear: both; margin-bottom: 0.5em; overflow:hidden;">
					...
					<a href="<?php echo rurl();?>/admin/image-manager.php?dirImgs=<?php echo $dirImgs?>&imgs=<?php echo $imgs?>&deleteImg=<?php echo $image;?>" style="color: red;">[DELETE]</a>
				</div>
			<?php }?>
		...
		<form action="<?php echo rurl();?>/admin/image-manager.php?dirImgs=<?php echo $dirImgs?>&imgs=<?php echo $imgs?>" method="post" enctype="multipart/form-data">
		...
		<?php if( (!empty($_POST['uploadSummary']) || !empty($_POST['uploadArticle'])) && empty($error) ){?>
			<div style="padding:5px; float: left;">
				...
				<div class="toolbar">
					<form id="results" method="post" action="<?php echo rurl();?>/admin/image-manager.php?dirImgs=<?php echo $dirImgs?>&imgs=<?php echo $imgs?>">
						...
						<?php }else{?>
						<input class="submit" name="submitCrop" type="submit" value="SAVE SELECTION"/>
