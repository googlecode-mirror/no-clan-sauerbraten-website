...
if ( !empty($_GET['idPost'])){
    ...
    if ($row['userId'] != $arrUser['idUser']) go_home();
...
if ( !empty($_POST['submitAdd']) || !empty ($_POST['submitEdit']) || !empty($_POST['submitPreview']) || !empty($_POST['submitReload']))
{
	...
	if (!empty($_POST['postFor']))
	    ...
	    if (!empty($_POST['submitEdit']))
	    {
		    $query= "UPDATE posts SET title = '$title', summary = '$summary', summary_img = '$summary_img',
			     body = '$body', date_pub = '$date_pub', tags = '$tags', postFor = '$postFor'  WHERE idPost = '$idPost'";
		    $result = mysql_query($query, $dbConn);
		    unset ($query, $result);
	    
		    header( 'Location: index.php?done=postEdit' );
		    die;
	    }
    }
}
...
<body style="background: url('../css/art/wrapperBackAdmin.png') no-repeat scroll center 0px; background-color: #000;">
	...
	<div class="pre_main">
		...
		<h1 class="title">
		    ...
		    <span class="info"><?php if (!empty($date_pub)) echo date("M j, Y", strtotime($date_pub));?></span>
		</h1>
		...
		<p class="info">by <a><?php echo $arrUser['username']?></a>, on <?php echo date("M j, Y", strtotime($date_pub));?>
	...
	<div id="container">
		...
		<div id="content">
			...
			<h3>
			    <a class="fancy_big_frame" href="<?php echo rurl().'/admin/image-manager.php?dirImgs='.$dirImgs.'&imgs='.$imgs;?>">
				[Image Manager]
			    </a>
			</h3>
			<!--/IMAGES -->
			...
			<form id="entry" name="entry" action="" method="post">
				...
				<a href="javascript:NewCssCal('demo1', 'yyyymmdd','dropdown',true,24,false)">
				    <input name="date_pub" id="demo1" style="width: 12em;" type="text" value="<?php canput($date_pub, date("Y-m-d H:i:s"));?>" />
				</a>
				<!-- /date_pub -->
				<br/>
				<!-- for -->
